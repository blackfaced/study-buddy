...
// 4*4*4 which is over the level).
//
// v0.8.x (PR #148): the source-of-truth fix is a `level` column on
// mistakes populated at insert time (server/src/mistake-level.ts).
// The picker now does a direct numeric compare (m.level <= kidLevel)
// — no client-side text heuristic. Pre-migration rows with no level
// are treated as "show anyway" so we never regress on legacy data.
// =====================================================================
...
test("pickGenWithBias: L3 mistake does NOT surface to L1 kid (the actual bug)", () => {
  ...
  // With the level column populated at insert time (PR #148), the server
  // sends `level: 3` so the picker can do a direct numeric compare.
  const l3Mistake = { id: 99, problem: "4 × 4 × 4", answer: 64, errorType: "vision_pending", level: 3 };
  let calls = 0;
...
test("pickGenWithBias: L3 mistake DOES surface to L3 kid (no over-blocking)", () => {
  const l3Mistake = { id: 99, problem: "4 × 4 × 4", answer: 64, errorType: "multiply", level: 3 };
  const pick = pickGenWithBias(
    items,
    { compute: 1, carry: 1, borrow: 1, multiply: 1 },
    () => 0.1,
  ...
  assert.equal(q.level, 3, "level field on the Question shape must reflect m.level");
});

test("pickGenWithBias: null level (legacy data) is treated as 'show anyway'", () => {
  // Pre-migration rows have no level. Picker must not regress to
  // over-blocking on legacy data — these rows still surface.
  const legacyMistake = { id: 100, problem: "5 + 7 = ?", answer: 12, errorType: "carry", level: null };
  const pick = pickGenWithBias(
    items,
    { compute: 1, carry: 1, borrow: 1, multiply: 1 },
    () => 0.1,
    { mistakeProvider: () => legacyMistake, mistakeRate: 1, levels: [1] },
  );
  const q = pick();
  assert.equal(q.fromMistake, true, "legacy null-level mistake must still surface (don't over-block)");
  assert.equal(q.level, null, "level field on the Question is null for legacy data");
});

test("pickGenWithBias: L2 mistake is skipped for L1 kid (not just L3)", () => {
  // After PR #148 the level column is the single source of truth.
  // L1 kid must skip L2 mistakes too (not just L3), not just the
  // old text-based "is multiply?" heuristic.
  const l2Mistake = { id: 101, problem: "35 + 27 = ?", answer: 62, errorType: "carry", level: 2 };
  let calls = 0;
  const provider = () => (calls++ === 0 ? l2Mistake : null);
  const pick = pickGenWithBias(
    items,
    { compute: 1, carry: 1, borrow: 1, multiply: 1 },
    () => 0.1,
    { mistakeProvider: provider, mistakeRate: 1, levels: [1] },
  );
  const q = pick();
  assert.equal(q.fromMistake, undefined, "L2 mistake must be skipped for L1 kid");
});
